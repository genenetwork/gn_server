defmodule GnServer.Router.Main do
  ...
  get "/species" do
    { :loaded, result } = Cachex.get(:gn_server_cache, conn.request_path, fallback: fn(_) ->
      Store.species
    end)
    json(conn, result)
  end
  ...
  namespace :trait do
    route_param :dataset, type: String do
      route_param :trait, type: String do
        get do
          { _, dataset } = integer_or_string(params[:dataset])
  ...
  namespace :trait do
    route_param :dataset, type: String do
      get do
        ...
        { :loaded, result } = Cachex.get(:gn_server_cache, conn.request_path,
          fallback: fn(_) ->
            Store.trait(dataset)
          end)
        case type do
